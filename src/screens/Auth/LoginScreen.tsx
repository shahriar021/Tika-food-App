...
const LoginScreen = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#B42315] items-center">
      ...
      <View className="flex-1 w-full  rounded-tr-2xl rounded-tl-2xl bg-white items-center p-4">
        <View className="mt-5 flex-row border border-gray-200 rounded-full overflow-hidden w-full p-1">
          ...
          <TouchableOpacity  className={`flex-1 rounded-full items-center p-3 ${isSignIn? "bg-white":" bg-[#C21A1E]"}`} onPress={()=>setIsSignIn(false)}>
            <Text className={`${isSignIn?"text-red-500":"text-white"}`}>Sign Up</Text>
          </TouchableOpacity>
        </View>

        {/* sign in */}
       {isSignIn? <View className="mt-4 flex-1 w-full ">
